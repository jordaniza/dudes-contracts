...
export async function deployRoulette(): Promise<Deployment> {
  console.log("Deploying Contracts HH");
  ...
  const rouletteFactory = await hre.ethers.getContractFactory(
    "Roulette",
    deployer,
  );

  const roulette = await hre.upgrades.deployProxy(rouletteFactory, [
    dudesToken.address,
    mockSeeder.address,
  ], {
    initializer: "initialize",
