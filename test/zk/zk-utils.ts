...
async function zkDeploy<C extends Contract>(contractName: string, deployer: ZkDeployer, args?: any[]): Promise<C> {
  const artifact = await deployer.loadArtifact(contractName);
  return (await deployer.deploy(artifact, args)) as C;
}

async function zkDeployProxy<C extends Contract>(contractName: string, deployer: ZkDeployer, args?: any[]): Promise<C> {
...
export async function deployRoulette(deployer: ZkDeployer): Promise<Deployment> {
  console.log("Deploying contracts with the account:", RICH_WALLET_PK);

  const mockRandomizer = await zkDeploy<MockRandomizer>("MockRandomizer", deployer);
  const dudesToken = await zkDeployProxy<Dude>("Dude", deployer);
  const roulette = await zkDeployProxy<Roulette>("Roulette", deployer, [dudesToken.address, mockRandomizer.address]);
